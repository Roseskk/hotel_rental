...
const HotelCard = ({room, cardHandler}) => {
    return(
        <div key={`${room.roomId}`} className={'flex relative flex-col font-primary text-default h-[400px] w-[30%] bg-red-700 rounded-[15px]'}>
            ...
            <button onClick={() => cardHandler(room)} className={'absolute w-[150px] text-center top-0 left-0 p-4 rounded-br-[10px] rounded-tl-[10px] bg-secondary text-small text-white'}>Подробнее</button>
